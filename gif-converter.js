...
(() => {
  ...
  async function convert(url, onProgress, signal, preferences = {}) {
    ...
    const releaseResources = () => {
      // Abort handlers run synchronously, even when Firefox is tearing down the
      // background page and an awaited conversion will never get another turn.
      if (worker) { try { worker.terminate(); } catch { /* Already terminated. */ } worker = null; }
      if (reader) { try { reader.cancel().catch(() => {}); } catch { /* Released reader. */ } reader = null; }
      if (video) {
        try { video.pause(); video.removeAttribute("src"); video.load(); } catch { /* Detached decoder. */ }
        video = null;
      }
