# X Feed Download — Firefox, версия 0.1.0

Пробная версия для настольного Firefox 115+. Маленькая иконка загрузки слева от «⋯» в заголовке поста. Серый цвет берётся у соседней кнопки, при наведении иконка становится синей.
...
- Это прототип: в среде разработки нет установленного Firefox и авторизованной ленты X. Проверены локальные тестовые ответы и имитация API расширений; интерфейс и живое скачивание в Firefox ещё не проверены.
...
Проверяются извлечение метаданных, лучший битрейт, цитаты и репосты, фильтрация доменов, побайтовая передача ответа, повреждённый JSON, ограничение размера, привязка загрузки к вкладке и известному посту.
